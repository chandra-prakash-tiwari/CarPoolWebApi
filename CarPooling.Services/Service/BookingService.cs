...
namespace CarPoolingEf.Services.Services
{
    public class BookingService :IBookingService
    {
        ...
        public bool CancelRideRequest(string id)
        {
            ...
            if (booking != null && booking.Status == BookingState.Pending)
            {
                booking.Status = BookingState.Cancel;
                return this.Db.SaveChanges() > 0;
            }
        ...
        {
            return this.Db.Bookings?.Where(a => a.BookerId == id && a.Status != BookingState.Completed).ToList();
        }

        public bool BookingResponse(string id, BookingState status)
        {
        ...
        {
            return this.Db.Bookings?.Where(booking => booking.Status == BookingState.Pending && booking.RideId == rideId).ToList();
        }
